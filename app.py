...
def _market_price_text(value: float | None, reference: float | None = None) -> str:
...
def _validation_direction_score(features: pd.DataFrame, market: pd.DataFrame, i: int) -> float:
...
def trend_transition_forecast(symbol_code: str, closed: pd.DataFrame, timeframe: str, cycle_context: str | None) -> dict:
    ...
    for candidate in candidates:
        ...
        # Best timing estimate for this direction. Each horizon gets its own
        ...
        market_ctx = broad_market_context(symbol_code, timeframe)
    ...
    # If both directions peak at the same time bucket, use the second-best
    ...
    # Build ONE chronological market path from the actual dominant scenario at
    ...
    # PRIMARY DIRECTION: compare UP vs DOWN directly.  The two evidence scores
...
def trend_chart(df: pd.DataFrame, symbol: str, timeframe: str, trend_info: dict):
    ...
    # ONE signal dot only. The dot is a live directional SIGNAL, not a pair of
...
if ui_mode == "Sencillo":
    ...
    def _render_fast_simple_mode():
        ...
        # The headline signal must be predictive, not merely the trend already
